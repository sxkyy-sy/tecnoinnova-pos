...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    // 1. Validar si el correo o el nombre ya existen en nuestra tabla de usuarios
    const { data: existingUser } = await supabase
      .from('usuarios')
      .select('nombre, email')
      .or(`email.eq.${email.trim()},nombre.eq.${nombre.trim()}`);

    if (existingUser && existingUser.length > 0) {
      toast.error('El nombre o correo ya están registrados por otro usuario.');
      setLoading(false);
      return;
    }
    ...
    if (authError) {
      ...
      // 3. Insertar el usuario validado en nuestra tabla 'usuarios'
      ...
      toast.success('Registro exitoso y cuenta validada.');
      navigate('/login');
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-color)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-10%', right: '-10%', width: '40vw', height: '40vw', background: 'rgba(16, 185, 129, 0.2)', filter: 'blur(100px)', borderRadius: '50%', opacity: 0.5 }}></div>
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h2 style={{ color: 'white', fontSize: '1.8rem', marginBottom: '0.5rem' }}>Crear Cuenta</h2>
          <p style={{ color: 'var(--text-muted)' }}>Únete al equipo de TecnoInnova</p>
        </div>

        <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div>
            <label style={{ display: 'block', color: 'var(--text-muted)', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Nombre Completo</label>
        ...
        <p style={{ textAlign: 'center', marginTop: '2rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
          ¿Ya tienes cuenta? <span style={{ color: 'var(--primary)', cursor: 'pointer' }} onClick={() => navigate('/login')}>Inicia Sesión</span>
        </p>
      </motion.div>
    </div>
  );
